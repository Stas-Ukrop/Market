// ./components/hooks/SnapshotEngine.js
...
export const takeFullSnapshot = async (symbol) => {
  try {
    ...
    // Некоторые браузеры могут создать подпапку, если передать "2023-10-27/..." но чаще заменяют слэш на подчеркивание.
    // Для надежности делаем префикс даты в имени.
    ...
    const canvas = await html2canvas(document.body, {
      ...
      scale: 1, // Можно увеличить до 2 для Retina качества, но файл будет тяжелее
      useCORS: true, // Важно для картинок с других доменов (если есть логотипы)
      logging: false,
    });
    ...
    // Звуковой эффект затвора камеры (опционально, для фидбека)
    playShutterSound();
